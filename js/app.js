...
let sortByDate = function (latter, previous) {
      return latter.timeStamp - previous.timeStamp
}
...
.then(() => {
  state.articles.sort((latter,previous) => {return latter.timeStamp-previous.timeStamp})
  let articleList = []
  let i = 0
  state.articles.forEach(article => {
